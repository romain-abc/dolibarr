 ...
 */
class pdf_livraison_simple extends ModelePDFCommandes
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "livraison_simple";
		$this->description = $langs->trans('PDFLivraisonSimpleDescription');
			...
			if (file_exists($dir)) {
				...
				for ($pageNo = 1; $pageNo <= 2; $pageNo++){
					...
					if($pageNo==1){
						...
						$tab_top_newpage = 90 + $top_shift + 6;
					}
		...
		*/
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_NO_RECIPENT_FRAME)) {
				$pdf->SetLineStyle(array('dash'=>0, 'color'=>array(128, 128, 128)));
				$pdf->SetTextColor(0, 0, 0);
				$pdf->SetFont('', '', $default_font_size - 2);
				$pdf->SetXY($posx + 2, $posy - 3);
				$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("BillTo"), 0, $ltrdirection);
				$pdf->Rect($posx, $posy+2, $widthrecbox, $hautcadre);
			}
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$largcol = ($this->page_largeur - $this->marge_droite - $posx)/2;
		$useborder = 0;
		$index = 0;
		// Total HT
		$pdf->SetFillColor(255, 255, 255);
		...
		$pdf->SetFont('', '', $default_font_size - 2);
		$pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("BLCustomerSignature"), 0, 'L', 1);

		$pdf->SetXY($posx, $tab_top + $tab_hl);
		$pdf->MultiCell($largcol, $tab_hl * 7, '', 1, 'R');
