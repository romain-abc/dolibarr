			...
			while ($i < $num) {
				...
				if (empty($reshook) && $forcedroundingmode == '0') {	// Check if data on line are consistent. This may solve lines that were not consistent because set with $forcedroundingmode='auto'
					...
					if($originLine->array_options['options_ecopart']){
						$tmpcalprice = $tmpcal[1] + (($originLine->array_options['options_ecopart']*$obj->qty)*0.2);
					}
					else{
						$tmpcalprice = $tmpcal[1];
					}

					$diff_when_using_price_ht = price2num((float) $tmpcalprice - $obj->total_tva, 'MT', 1); // If price was set with tax price and unit price HT has a low number of digits, then we may have a diff on recalculation from unit price HT.
