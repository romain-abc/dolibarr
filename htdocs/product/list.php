...
$arrayfields = array(
	...
	'p.description'=>array('label'=>"Description", 'checked'=>1, 'position'=>10),
...
// --------------------------------------------------------------------
$sql = 'SELECT p.rowid, p.ref, p.label, p.description, p.fk_product_type, p.barcode, p.price, p.tva_tx, p.price_ttc, p.price_base_type, p.entity,';
...
$sql .= $hookmanager->resPrint;
$sql .= " GROUP BY p.rowid, p.ref, p.label, p.description, p.barcode, p.price, p.tva_tx, p.price_ttc, p.price_base_type,";
$sql .= " p.fk_product_type, p.duration, p.finished, p.tosell, p.tobuy, p.seuil_stock_alerte, p.desiredstock,";
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		// Description
		if (!empty($arrayfields['p.description']['checked'])) {
			print '<td class="tdoverflowmax200" title="'.dol_escape_htmltag($product_static->description).'">'.$product_static->description.'</td>';
			if (!$i) {
				$totalarray['nbfield']++;
			}
		}
